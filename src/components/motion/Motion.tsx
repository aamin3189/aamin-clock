...
const Motion = () => {
  ...
  async function setLocationData() {
    ...
    await getPrayerTimes(data.latitude, data.longitude);
  }
  ...
  async function getPrayerTimes(latitude: number, longitude: number) {
    ...
    setPrayerTimes({
      ...
      sunRiseHour: moment(pt.sunrise).format("h"),
    });
    return pt;
  }
  ...
  function changeScrollPosition() {
    ...
    // Assuming sunrise is at 6:00 AM and midnight is at 12:00 AM
    var sunriseHour = prayerTimes?.sunRiseHour ?? 6;
    var midnightHour = 0;

    if (currentHour >= sunriseHour && currentHour < midnightHour) {
      scrollPosition = (currentHour - sunriseHour) * (5500 / (midnightHour - sunriseHour));
    } else {
      scrollPosition = 5500;
    }
  ...
  useEffect(() => {
    setInterval(changeScrollPosition, 3600000);
  ...
  return (
    <div className="motion-wrap">
      ...
      <h2 className="main-time-motion">{moment(time).format("hh:mm")}</h2>
    </div>
  );
};
